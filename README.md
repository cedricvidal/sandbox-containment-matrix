...
| Needs privilege to run? | no | no — but strip container MAC/seccomp/`proc` masking (`label=disable`, `apparmor=unconfined`, `seccomp=unconfined`, `systempaths=unconfined`; findings §23) | **yes** — `privileged: true` (bwrap can't map uid in k8s userns) | n/a (Fly runs the VM) |
